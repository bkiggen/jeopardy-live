...
describe('POST /api/players', () => {
  it('creates a player at score 0', async () => {
    const res = await request(app).post('/api/players').send({ name: name('alice') });
...
describe('GET /api/players', () => {
  it('lists active players with their current-season score', async () => {
    const created = await request(app).post('/api/players').send({ name: name('lister') });
...
describe('PATCH /api/players/:id', () => {
  ...
  it('400s when isActive is missing', async () => {
    const created = await request(app).post('/api/players').send({ name: name('badpatch') });
    const res = await request(app).patch(`/api/players/${created.body.id}`).send({});
    expect(res.status).toBe(400);
  });
});
